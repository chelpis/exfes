...
// generated with L = 8
void exhaustive_sse2_deg_4_T_4_el_0(LUT_t LUT, int n, pck_vector_t nonsimd_F[], solution_callback_t callback, void* callback_state, int verbose) {
    ...
    for(int idx_1=0; idx_1<idx_0; idx_1++) {
      ...
      for(int idx_2=0; idx_2<idx_1; idx_2++) {
        ...
        for(uint64_t j=256; j<(1ull << idx_2); j+=256) {
              ...
              func_deg_4_T_4_el_0(F, &F_sp[0], solution_buffer, &n_solutions_found, i);
