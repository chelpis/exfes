...
// generated with L = 8
void exhaustive_sse2_deg_5_T_2_el_0(LUT_t LUT, int n, pck_vector_t nonsimd_F[], solution_callback_t callback, void* callback_state, int verbose) {
  const __m128i simd_zero = _mm_setzero_si128();

  // *****initialise an SIMD version of the constants and internal state
  const int N = n_monomials(n, 5);
  simd_t * F = NULL;
      ...
      for(int idx_2=0; idx_2<idx_1; idx_2++) {
        ...
        for(int idx_3=0; idx_3<idx_2; idx_3++) {
          ...
          for(uint64_t j=256; j<(1ull << idx_3); j+=256) {
                ...
                uint64_t F_sp[ 4 ] = { alpha*16, beta*16, gamma*16, delta*16 };
                func_deg_5_T_2_el_0(F, &F_sp[0], solution_buffer, &n_solutions_found, i);
