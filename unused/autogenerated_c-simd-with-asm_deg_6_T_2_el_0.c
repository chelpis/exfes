...
// generated with L = 7
void exhaustive_sse2_deg_6_T_2_el_0(LUT_t LUT, int n, pck_vector_t nonsimd_F[], solution_callback_t callback, void* callback_state, int verbose) {
  const __m128i simd_zero = _mm_setzero_si128();

  // *****initialise an SIMD version of the constants and internal state
  const int N = n_monomials(n, 6);
  simd_t * F = NULL;
        ...
        for(int idx_3=0; idx_3<idx_2; idx_3++) {
          ...
          for(int idx_4=0; idx_4<idx_3; idx_4++) {
            ...
            for(uint64_t j=128; j<(1ull << idx_4); j+=128) {
                  ...
                  uint64_t F_sp[ 5 ] = { alpha*16, beta*16, gamma*16, delta*16, epsilon*16 };
                  func_deg_6_T_2_el_0(F, &F_sp[0], solution_buffer, &n_solutions_found, i);
