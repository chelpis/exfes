...
// generated with L = 6
void exhaustive_sse2_deg_9_T_4_el_0(LUT_t LUT, int n, pck_vector_t nonsimd_F[], solution_callback_t callback, void* callback_state, int verbose) {
        ...
        for(int e=0; e<f; e++)
          for(int d=0; d<e; d++)
            for(int c=0; c<d; c++)
              for(int b=0; b<c; b++)
                for(int a=0; a<b; a++)
                  F[ 0 ] ^= F[ idx_8(LUT, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e, n-4+f, n-4+g, n-4+h) ] & (v[a] & v[b] & v[c] & v[d] & v[e] & v[f] & v[g] & v[h]);
        ...
        for(int e=0; e<f; e++)
          for(int d=0; d<e; d++)
            for(int c=0; c<d; c++)
              for(int b=0; b<c; b++)
                for(int a=0; a<b; a++)
                  F[ idx_1(LUT, idx_0) ] ^= F[ idx_8(LUT, idx_0, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e, n-4+f, n-4+g) ] & (v[a] & v[b] & v[c] & v[d] & v[e] & v[f] & v[g]);
        ...
        for(int e=0; e<f; e++)
          for(int d=0; d<e; d++)
            for(int c=0; c<d; c++)
              for(int b=0; b<c; b++)
                for(int a=0; a<b; a++)
                  F[ idx_2(LUT, idx_0, idx_1) ] ^= F[ idx_8(LUT, idx_0, idx_1, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e, n-4+f) ] & (v[a] & v[b] & v[c] & v[d] & v[e] & v[f]);
        ...
        for(int e=0; e<4; e++)
          for(int d=0; d<e; d++)
            for(int c=0; c<d; c++)
              for(int b=0; b<c; b++)
                for(int a=0; a<b; a++)
                  F[ idx_3(LUT, idx_0, idx_1, idx_2) ] ^= F[ idx_8(LUT, idx_0, idx_1, idx_2, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e) ] & (v[a] & v[b] & v[c] & v[d] & v[e]);
        ...
        for(int idx_0=0; idx_0<idx_1; idx_0++)
          for(int d=0; d<4; d++)
            for(int c=0; c<d; c++)
              for(int b=0; b<c; b++)
                for(int a=0; a<b; a++)
                  F[ idx_4(LUT, idx_0, idx_1, idx_2, idx_3) ] ^= F[ idx_8(LUT, idx_0, idx_1, idx_2, idx_3, n-4+a, n-4+b, n-4+c, n-4+d) ] & (v[a] & v[b] & v[c] & v[d]);
        ...
        for(int idx_1=0; idx_1<idx_2; idx_1++)
          for(int idx_0=0; idx_0<idx_1; idx_0++)
            for(int c=0; c<4; c++)
              for(int b=0; b<c; b++)
                for(int a=0; a<b; a++)
                  F[ idx_5(LUT, idx_0, idx_1, idx_2, idx_3, idx_4) ] ^= F[ idx_8(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, n-4+a, n-4+b, n-4+c) ] & (v[a] & v[b] & v[c]);
        ...
        for(int idx_2=0; idx_2<idx_3; idx_2++)
          for(int idx_1=0; idx_1<idx_2; idx_1++)
            for(int idx_0=0; idx_0<idx_1; idx_0++)
              for(int b=0; b<4; b++)
                for(int a=0; a<b; a++)
                  F[ idx_6(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5) ] ^= F[ idx_8(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, n-4+a, n-4+b) ] & (v[a] & v[b]);
        ...
        for(int idx_3=0; idx_3<idx_4; idx_3++)
          for(int idx_2=0; idx_2<idx_3; idx_2++)
            for(int idx_1=0; idx_1<idx_2; idx_1++)
              for(int idx_0=0; idx_0<idx_1; idx_0++)
                for(int a=0; a<4; a++)
                  F[ idx_7(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, idx_6) ] ^= F[ idx_8(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, idx_6, n-4+a) ] & (v[a]);
          ...
          for(int e=0; e<f; e++)
            for(int d=0; d<e; d++)
              for(int c=0; c<d; c++)
                for(int b=0; b<c; b++)
                  for(int a=0; a<b; a++)
                    F[ 0 ] ^= F[ idx_9(LUT, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e, n-4+f, n-4+g, n-4+h, n-4+i) ] & (v[a] & v[b] & v[c] & v[d] & v[e] & v[f] & v[g] & v[h] & v[i]);
          ...
          for(int e=0; e<f; e++)
            for(int d=0; d<e; d++)
              for(int c=0; c<d; c++)
                for(int b=0; b<c; b++)
                  for(int a=0; a<b; a++)
                    F[ idx_1(LUT, idx_0) ] ^= F[ idx_9(LUT, idx_0, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e, n-4+f, n-4+g, n-4+h) ] & (v[a] & v[b] & v[c] & v[d] & v[e] & v[f] & v[g] & v[h]);
          ...
          for(int e=0; e<f; e++)
            for(int d=0; d<e; d++)
              for(int c=0; c<d; c++)
                for(int b=0; b<c; b++)
                  for(int a=0; a<b; a++)
                    F[ idx_2(LUT, idx_0, idx_1) ] ^= F[ idx_9(LUT, idx_0, idx_1, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e, n-4+f, n-4+g) ] & (v[a] & v[b] & v[c] & v[d] & v[e] & v[f] & v[g]);
          ...
          for(int e=0; e<f; e++)
            for(int d=0; d<e; d++)
              for(int c=0; c<d; c++)
                for(int b=0; b<c; b++)
                  for(int a=0; a<b; a++)
                    F[ idx_3(LUT, idx_0, idx_1, idx_2) ] ^= F[ idx_9(LUT, idx_0, idx_1, idx_2, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e, n-4+f) ] & (v[a] & v[b] & v[c] & v[d] & v[e] & v[f]);
          ...
          for(int e=0; e<4; e++)
            for(int d=0; d<e; d++)
              for(int c=0; c<d; c++)
                for(int b=0; b<c; b++)
                  for(int a=0; a<b; a++)
                    F[ idx_4(LUT, idx_0, idx_1, idx_2, idx_3) ] ^= F[ idx_9(LUT, idx_0, idx_1, idx_2, idx_3, n-4+a, n-4+b, n-4+c, n-4+d, n-4+e) ] & (v[a] & v[b] & v[c] & v[d] & v[e]);
          ...
          for(int idx_0=0; idx_0<idx_1; idx_0++)
            for(int d=0; d<4; d++)
              for(int c=0; c<d; c++)
                for(int b=0; b<c; b++)
                  for(int a=0; a<b; a++)
                    F[ idx_5(LUT, idx_0, idx_1, idx_2, idx_3, idx_4) ] ^= F[ idx_9(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, n-4+a, n-4+b, n-4+c, n-4+d) ] & (v[a] & v[b] & v[c] & v[d]);
          ...
          for(int idx_1=0; idx_1<idx_2; idx_1++)
            for(int idx_0=0; idx_0<idx_1; idx_0++)
              for(int c=0; c<4; c++)
                for(int b=0; b<c; b++)
                  for(int a=0; a<b; a++)
                    F[ idx_6(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5) ] ^= F[ idx_9(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, n-4+a, n-4+b, n-4+c) ] & (v[a] & v[b] & v[c]);
          ...
          for(int idx_2=0; idx_2<idx_3; idx_2++)
            for(int idx_1=0; idx_1<idx_2; idx_1++)
              for(int idx_0=0; idx_0<idx_1; idx_0++)
                for(int b=0; b<4; b++)
                  for(int a=0; a<b; a++)
                    F[ idx_7(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, idx_6) ] ^= F[ idx_9(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, idx_6, n-4+a, n-4+b) ] & (v[a] & v[b]);
          ...
          for(int idx_3=0; idx_3<idx_4; idx_3++)
            for(int idx_2=0; idx_2<idx_3; idx_2++)
              for(int idx_1=0; idx_1<idx_2; idx_1++)
                for(int idx_0=0; idx_0<idx_1; idx_0++)
                  for(int a=0; a<4; a++)
                    F[ idx_8(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, idx_6, idx_7) ] ^= F[ idx_9(LUT, idx_0, idx_1, idx_2, idx_3, idx_4, idx_5, idx_6, idx_7, n-4+a) ] & (v[a]);
              ...
              for(int idx_6=0; idx_6<idx_5; idx_6++) {
                ...
                for(int idx_7=0; idx_7<idx_6; idx_7++) {
                  ...
                  for(uint64_t j=64; j<(1ull << idx_7); j+=64) {
                        ...
                        func_deg_9_T_4_el_0(F, &F_sp[0], solution_buffer, &n_solutions_found, i);
