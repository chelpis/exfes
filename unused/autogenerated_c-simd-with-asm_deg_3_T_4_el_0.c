...
// generated with L = 9
void exhaustive_sse2_deg_3_T_4_el_0(LUT_t LUT, int n, pck_vector_t nonsimd_F[], solution_callback_t callback, void* callback_state, int verbose) {
  ...
  for(int idx_0=0; idx_0<n-4; idx_0++) {
    ...
    for(int idx_1=0; idx_1<idx_0; idx_1++) {
      ...
      for(uint64_t j=512; j<(1ull << idx_1); j+=512) {
            ...
            func_deg_3_T_4_el_0(F, &F_sp[0], solution_buffer, &n_solutions_found, i);
