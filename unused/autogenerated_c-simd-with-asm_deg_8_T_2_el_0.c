...
// generated with L = 6
void exhaustive_sse2_deg_8_T_2_el_0(LUT_t LUT, int n, pck_vector_t nonsimd_F[], solution_callback_t callback, void* callback_state, int verbose) {
  const __m128i simd_zero = _mm_setzero_si128();

  // *****initialise an SIMD version of the constants and internal state
  const int N = n_monomials(n, 8);
  simd_t * F = NULL;
            ...
            for(int idx_5=0; idx_5<idx_4; idx_5++) {
              ...
              for(int idx_6=0; idx_6<idx_5; idx_6++) {
                ...
                for(uint64_t j=64; j<(1ull << idx_6); j+=64) {
                      ...
                      uint64_t F_sp[ 7 ] = { alpha*16, beta*16, gamma*16, delta*16, epsilon*16, zeta*16, eta*16 };
                      func_deg_8_T_2_el_0(F, &F_sp[0], solution_buffer, &n_solutions_found, i);
